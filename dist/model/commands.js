...
import { promptChoice, promptSelect, promptText } from "../setup/prompts.js";
...
export function resolveModelProviderForCommand(authPath, input) {
...
async function selectApiKeyProvider() {
    const options = API_KEY_PROVIDERS.map((provider) => ({
        value: provider,
...
function maybeSetRecommendedDefaultModel(settingsPath, authPath) {
    if (!settingsPath) {
        return;
    }

...
export const cliCommandSections = [
	{
		...
		commands: [
			...
			{ usage: "lex setup preview", description: "Install or verify preview dependencies." },
			{ usage: "lex doctor", description: "Diagnose config, auth, Pi runtime, and preview dependencies." },
			{ usage: "lex status", description: "Show the current setup summary." },
		],
	},
	{
		title: "Model Management",
		commands: [
			{ usage: "lex model list", description: "List available models in Pi auth storage." },
			{ usage: "lex model login [id]", description: "Authenticate a model provider with OAuth or API-key setup." },
			{ usage: "lex model logout [id]", description: "Clear stored auth for a model provider." },
			{ usage: "lex model set <provider/model>", description: "Set the default model (also accepts provider:model)." },
			{ usage: "lex model tier [value]", description: "View or set the request service tier override." },
		],
	},
	{
		title: "AlphaXiv",
		commands: [
			{ usage: "lex alpha login", description: "Sign in to alphaXiv (legacy)." },
			{ usage: "lex alpha logout", description: "Clear alphaXiv auth." },
			{ usage: "lex alpha status", description: "Check alphaXiv auth status." },
		],
	},
	{
		title: "Utilities",
		commands: [
			{ usage: "lex packages list", description: "Show core and optional Pi package presets." },
			{ usage: "lex packages install <preset>", description: "Install optional package presets on demand." },
			{ usage: "lex search status", description: "Show Pi web-access status and config path." },
			{ usage: "lex search set <provider> [api-key]", description: "Set the web search provider and optionally save its API key." },
			{ usage: "lex search clear", description: "Reset web search provider to auto while preserving API keys." },
			{ usage: "lex update [package]", description: "Update installed packages, or a specific package." },
		],
	},
];
...
export const legacyFlags = [
	...
	{ usage: "--alpha-login", description: "Sign in to alphaXiv (legacy) and exit." },
...
export function formatCliWorkflowUsage(command) {
	return `lex ${command.name}${command.args ? ` ${command.args}` : ""}`;
}

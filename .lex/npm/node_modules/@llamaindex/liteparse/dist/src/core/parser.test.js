...
const { mockPages, mockPdfDocument, mockParsedPages, mockBoundingBoxes, mockPdfConvertedResult, mockOcrResults, mockParseResultJson, mockParsedPagesOcr, mockParseResultJsonOcr, mockTextItems, mockParsedPagesWithBoundingBoxes, mockParsedPagesWithBoundingBoxesOcr, mockScreenshotResults, } = vi.hoisted(() => {
...
vi.mock("../conversion/convertToPdf.js", async () => {
    const actual = await vi.importActual("../conversion/convertToPdf.js");
    return {
        ...actual,
...
vi.mock("../engines/pdf/pdfjs.js", async () => {
    const actual = await vi.importActual("../engines/pdf/pdfjs.js");
    return {
        ...actual,
        PdfJsEngine: vi.fn(class {
            constructor() { }
...
vi.mock("../engines/ocr/tesseract.js", async () => {
    const actual = await vi.importActual("../engines/ocr/tesseract.js");
    return {
        ...actual,
        TesseractEngine: vi.fn(class {
            constructor() { }
...
vi.mock("../engines/ocr/http-simple.js", async () => {
    const actual = await vi.importActual("../engines/ocr/http-simple.js");
    return {
        ...actual,
        HttpOcrEngine: vi.fn(class {
            url;
            constructor(url) {
                this.url = url;
                if (this.url != "http://localhost:8000") {
...
vi.mock("../processing/grid.js", async () => {
    const actual = await vi.importActual("../processing/grid.js");
    return {
        ...actual,
...
vi.mock("../processing/bbox.js", async () => {
    const actual = await vi.importActual("../processing/bbox.js");
    return {
        ...actual,
...
vi.mock("../engines/pdf/pdfium-renderer.js", async () => {
    const actual = await vi.importActual("../engines/pdf/pdfium-renderer.js");
    return {
        ...actual,
...
describe("Parse tests", () => {
    it("test parse without OCR and text format", async () => {
        const config = { ocrEnabled: false, outputFormat: "text" };
    ...
    it("test parse without OCR and json format", async () => {
        const config = { ocrEnabled: false, outputFormat: "json" };
        const liteparse = new LiteParse(config);
        expect(liteparse.getConfig().ocrEnabled).toBeFalsy();
    ...
    it("test parse with OCR (tesseract) and text format", async () => {
        ...
        expect(result.pages.filter((page) => {
    ...
    it("test parse with OCR (tesseract) and json format", async () => {
        ...
        expect(result.pages.filter((page) => {
            return page.textItems.length > 0;
        }).length).toBe(result.pages.length);

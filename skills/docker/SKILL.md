...
Run research code inside Docker containers while Lex stays on the host. The container gets the project files, runs the commands, and results sync back.
...
```bash
docker build -t lex-experiment .
docker run --rm -v "$(pwd)/results":/workspace/results lex-experiment
```

...
function resolveLexAgentDir(): string {
	const configured = process.env.PI_CODING_AGENT_DIR ?? process.env.LEX_CODING_AGENT_DIR;
	if (configured?.trim()) {
		return resolve(expandHomePath(configured.trim()));
	}
	return resolve(homedir(), ".lex", "agent");
}
...
export function registerLexModelCommand(pi: ExtensionAPI): void {
	pi.registerCommand("lex-model", {
		description: "Open Lex model menu (main + per-subagent overrides).",
		handler: async (_args, ctx) => {
			if (!ctx.hasUI) {
				ctx.ui.notify("lex-model requires interactive mode.", "error");
				return;
			}
			...
			try {
				...
				const agentDir = resolveLexAgentDir();

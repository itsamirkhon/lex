...
import { APP_ROOT, LEX_AGENT_LOGO, LEX_VERSION } from "./shared.ts";
...
function getRecentActivitySummary(ctx: ExtensionContext): string {
	...
	for (let index = branch.length - 1; index >= 0; index -= 1) {
		...
		if (!text) continue;
		const role = msg.message.role === "assistant" ? "lex" : msg.message.role === "user" ? "you" : msg.message.role;
		return `${role}: ${text}`;
	}
	return "";
}
...
async function buildAgentCatalogSummary(): Promise<{ agents: string[]; chains: string[] }> {
	...
	const chains: string[] = [];
	try {
		const entries = await readdir(resolvePath(APP_ROOT, ".lex", "agents"), { withFileTypes: true });
...
function detectSystemResources(): SystemResources {
	...
	cachedResources = { cpu: "", cores, ramTotal, docker: false };
	try {
...
function getLegalWorkflows(pi: ExtensionAPI): { name: string; description: string }[] {
...
export function installLexHeader(
	pi: ExtensionAPI,
	...
	cacheObj.agentSummaryPromise ??= buildAgentCatalogSummary();

	return cacheObj.agentSummaryPromise.then((agentData) => {
		const resources = detectSystemResources();
		const workflows = getLegalWorkflows(pi);
		const toolCount = pi.getAllTools().length;
		const commandCount = pi.getCommands().length;
		const agentCount = agentData.agents.length;
		...
		ctx.ui.setHeader((_tui, theme) => ({
			render(width: number): string[] {
				...
				push("");
				if (cardW >= 70) {
					const maxLogoW = Math.max(...LEX_AGENT_LOGO.map((l) => l.length));
					const logoOffset = " ".repeat(Math.max(0, Math.floor((cardW - maxLogoW) / 2)));
					for (const logoLine of LEX_AGENT_LOGO) {
						push(theme.fg("accent", theme.bold(`${logoOffset}${truncateVisible(logoLine, cardW)}`)));
					}
					push("");
				}

				const versionTag = ` Lex v${LEX_VERSION} — BMW Legal AI `;
				...
				if (useWideLayout) {
					...
					if (agentData.agents.length > 0) {
						leftLines.push("");
						leftLines.push(theme.fg("accent", theme.bold("Legal Agents")));
						for (const line of wrapWords(agentData.agents.join(", "), leftW)) {
					...
					const rightLines: string[] = [
						"",
						theme.fg("accent", theme.bold("Legal Workflows")),
					];

					for (const wf of workflows) {
						const descWords = wf.description.split(" ");
						let line = "";
						let first = true;
					...
					push(sep());
					push(row(theme.fg("accent", theme.bold("Legal Workflows"))));
					const narrowDescW = Math.max(1, contentW - 22);
					for (const wf of workflows) {
						push(row(`${theme.fg("accent", wf.name.padEnd(21))} ${theme.fg("dim", truncateVisible(wf.description, narrowDescW))}`));
					}

					if (agentData.agents.length > 0) {
						push(sep());
						push(row(theme.fg("accent", theme.bold("Legal Agents"))));
						push(row(theme.fg("dim", truncateVisible(agentData.agents.join(", "), contentW))));
					}
				}

...
export function registerDiscoveryCommands(pi: ExtensionAPI): void {
	pi.registerCommand("commands", {
		description: "Browse all available slash commands, including legal workflows and package commands.",
	...
	pi.registerCommand("capabilities", {
		description: "Show installed packages, legal workflows, and runtime capability counts.",
		handler: async (_args, ctx) => {
			...
			const items = [
				...
				`Legal Workflows: ${workflows.length}`,
				`Tools: ${tools.length}`,
				`Packages: ${packages.length}`,
				"--- Discovery ---",
				"/commands — browse slash commands",
				"/tools — inspect callable tools",
				"/matters — browse open legal matters",
				"/outputs — browse all legal artifacts",
				"--- Installed Packages ---",
